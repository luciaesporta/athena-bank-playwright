...
test('TC2 - Log in successfully with redirection to the Dashboard', async ({ pageAuth, pageDashboard, page }) => {
  ...
  await pageAuth.loginAndRedirectionToDashboardPage(validUser.email, validUser.password);
...
test('TC3 - Routes are protected for non-authenticated users', async ({ pageAuth }) => {
  ...
  await pageAuth.navigationFailsWhenUserIsLoggedout(validUser.email, validUser.password);
...
test('TC4 - Login fails introducing wrong password', async ({ pageAuth, page }) => {
  ...
  await pageAuth.loginFailsIntroducingWrongPassword(validUser.email, 'wrongpassword');
...
test('TC5 - Login fails when email field is empty', async ({ pageAuth }) => {
  ...
  await pageAuth.submitEmptyEmailLoginFormShouldFail(validUser.password);
